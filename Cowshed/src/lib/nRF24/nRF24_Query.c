...
uint8_t *nrfQuery(uint8_t type, uint8_t opcode,void *bufPtr, uint8_t len)
{
    ...
    do
    {
        ...
        if (queryPtr != NULL)
        {
            if(_nrfDebug){SerialPrintlnF(P("Query Successful!"));}
            break;
        }
...
//   addr_t addr;
  ...
  if (addrPtr != NULL)
  {
    ...
    {
        SerialPrintlnF(P("Checksum matched"));
        // memcpy(addrBuf,addr.node, sizeof(addr.node));
    }
    else
    {
       SerialPrintlnF(P("Checksum not matched"));
    }
  ...
  {
    SerialPrintF(P("Addr Rcv Failed"));
    // return NULL;
  } 
}
...
bool nrfTxHasAddr(addr_t *addrPtr, addrFun_t read)
{
    ...
    SerialPrintlnF(P("Device hs no address"));
    return false;
}
...
void nrfTxAddrHandler(addrFun_t read, addrFun_t save)
{
    ...
    addr_t addr;
    // read(&addr);
    // if(addr.type == PING_TYPE && addr.opcode == ADDR_OPCODE)
    // {
    //     SerialPrintlnF(P("Device has addr in memory"));
    //     // memcpy(_txAddr,addr,node,sizeof(_txAddr));
    // }
    bool hasAddr = nrfTxHasAddr(&addr,read);
    if(hasAddr == false)
    {
        SerialPrintlnF(P("Getting Addr from BS"));
        nrfGetAddr(&addr);
        save(&addr);
    }
    //validate address using checksum 
    memcpy(_txAddr,addr.node,sizeof(addr.node));
    SerialPrintlnF(P("TX Addr : ")); 
