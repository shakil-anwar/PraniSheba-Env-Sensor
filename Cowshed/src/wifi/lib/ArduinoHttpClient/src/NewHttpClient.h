...
#ifndef NewHttpClient_h
#define NewHttpClient_h
...
// This call was made when the NewHttpClient class wasn't expecting it
...
class NewHttpClient : public Client
{
public:
...
// FIXME Update tempToPachube example to calculate Content-Length correctly
    ...
    NewHttpClient(Client& aClient, const char* aServerName, uint16_t aServerPort = kHttpPort);
    NewHttpClient(Client& aClient, const String& aServerName, uint16_t aServerPort = kHttpPort);
    NewHttpClient(Client& aClient, const IPAddress& aServerAddress, uint16_t aServerPort = kHttpPort);
