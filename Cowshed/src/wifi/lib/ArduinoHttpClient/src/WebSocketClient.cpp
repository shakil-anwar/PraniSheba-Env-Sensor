...
WebSocketClient::WebSocketClient(Client& aClient, const char* aServerName, uint16_t aServerPort)
 : NewHttpClient(aClient, aServerName, aServerPort),
   iTxStarted(false),
   iRxSize(0)
{
}
...
WebSocketClient::WebSocketClient(Client& aClient, const IPAddress& aServerAddress, uint16_t aServerPort)
 : NewHttpClient(aClient, aServerAddress, aServerPort),
   iTxStarted(false),
   iRxSize(0)
{
}
...
int WebSocketClient::endMessage()
{
    ...
    NewHttpClient::write(0x80 | iTxMessageType);

    // the message is masked (0x80)
    // send the length
    if (iTxSize < 126)
    {
        NewHttpClient::write(0x80 | (uint8_t)iTxSize);
    }
    else if (iTxSize < 0xffff)
    {
        NewHttpClient::write(0x80 | 126);
        NewHttpClient::write((iTxSize >> 8) & 0xff);
        NewHttpClient::write((iTxSize >> 0) & 0xff);
    }
    else
    {
        NewHttpClient::write(0x80 | 127);
        NewHttpClient::write((iTxSize >> 56) & 0xff);
        NewHttpClient::write((iTxSize >> 48) & 0xff);
        NewHttpClient::write((iTxSize >> 40) & 0xff);
        NewHttpClient::write((iTxSize >> 32) & 0xff);
        NewHttpClient::write((iTxSize >> 24) & 0xff);
        NewHttpClient::write((iTxSize >> 16) & 0xff);
        NewHttpClient::write((iTxSize >> 8) & 0xff);
        NewHttpClient::write((iTxSize >> 0) & 0xff);
    }
    ...
    NewHttpClient::write(maskKey, sizeof(maskKey));
    ...
    return (NewHttpClient::write(iTxBuffer, txSize) == txSize) ? 0 : 1;
}
...
size_t WebSocketClient::write(const uint8_t *aBuffer, size_t aSize)
{
    if (iState < eReadingBody)
    {
        ...
        return NewHttpClient::write(aBuffer, aSize);
    }
...
int WebSocketClient::parseMessage()
{
    ...
    // are available
    if (NewHttpClient::available() < 2)
    {
        return 0;
    }

    // read open code and length
    uint8_t opcode = NewHttpClient::read();
    int length = NewHttpClient::read();
    ...
    {
        iRxSize = (NewHttpClient::read() << 8) | NewHttpClient::read();
    }
    else
    {
        iRxSize = ((uint64_t)NewHttpClient::read() << 56) | 
                    ((uint64_t)NewHttpClient::read() << 48) | 
                    ((uint64_t)NewHttpClient::read() << 40) | 
                    ((uint64_t)NewHttpClient::read() << 32) | 
                    ((uint64_t)NewHttpClient::read() << 24) | 
                    ((uint64_t)NewHttpClient::read() << 16) | 
                    ((uint64_t)NewHttpClient::read() << 8)  |
                    (uint64_t)NewHttpClient::read(); 
    }
    ...
    if (iRxMasked)
    {
        ...
        {
            iRxMaskKey[i] = NewHttpClient::read();
        }
    }
...
int WebSocketClient::available()
{
    if (iState < eReadingBody)
    {
        return NewHttpClient::available();
    }
...
{
    int readCount = NewHttpClient::read(aBuffer, aSize);
