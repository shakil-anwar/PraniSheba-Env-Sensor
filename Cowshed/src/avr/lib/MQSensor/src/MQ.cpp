...
float MQ::calculateRs()
{
  return _calculateRs();
}

void MQ::attachSaveFuc(funCalib_t save)
{
  if(save != NULL)
  {
    saveCalib = save;
...
void MQ::externCalibrate(float ppmVal)
{
  float ppm_log = log10(ppmVal);
  float ratio_RS_R0 = ppm_log*_calib.m + _calib.c;
  ratio_RS_R0 = pow(10,ratio_RS_R0);
  _calib.R0 = _calculateRs()/ratio_RS_R0;
  _calib.done  = true;
  if(saveCalib != NULL)
  {
...
float MQ::_calculateRs()
{
  ...
  uint32_t adc1 = analogRead(_analogPin);
  float adc = ((adc1*3.3/4095) + 0.18);
  adc = adc*3;
#else
  ...
  float RS = ((5.0 - Vrs) * _calib.RL) / Vrs; //deduced from voltage divider equation
