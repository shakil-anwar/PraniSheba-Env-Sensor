...
public:
    ...
    void setUnixTime(uint32_t unixtime)
    {
        ...
        _wire.write(Uint8ToBcd(t.tm_sec));
        _wire.write(Uint8ToBcd(t.tm_min));
        _wire.write(Uint8ToBcd(t.tm_hour)); // 24 hour mode only
        _wire.write(Uint8ToBcd(t.tm_mday));
        _wire.write(Uint8ToBcd(t.tm_mon+1));
        ...
        _wire.write(Uint8ToBcd(t.tm_year + 1900 - 2000));
        _wire.write(0); // no write protect, as all of this is ignored if it is protected
