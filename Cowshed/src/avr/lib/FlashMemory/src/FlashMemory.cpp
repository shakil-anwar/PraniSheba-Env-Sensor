...
#define mySPI SPI
#elif defined(ARDUINO_ARCH_ESP32) || defined(ESP32_V010)
#define mySPI SPI
...
#if defined(ARDUINO_ARCH_AVR) || defined(ARDUINO_ARCH_ESP32) || defined(ESP32_V010)
...
void Flash::readPage(uint32_t pageAddr, uint8_t *buf)
{
...
uint8_t Flash::readMFID(void)
{
	uint8_t ptr[3];
	// _busyWait();
	csLow();
	mySPI.transfer(FLASH_JEDEC_ID);
	for (uint16_t i = 0; i < 3; i++)
	{
		ptr[i] = mySPI.transfer(0);
	}
	csHigh();
	// Serial.print(F("Flash MID: "));
	// Serial.print(ptr[0]);
...
bool Flash::memoryCheck(uint32_t startAddr, uint32_t endAddr)
{
	bool memoryOk = true;
	uint8_t readPage[256];
	uint8_t writePage[256];
	uint32_t startSector = startAddr>>12;
	uint32_t endSector = endAddr>>12;
	uint32_t totalSector = endSector -  startSector + 1;
	uint32_t  currentPage = startAddr>>8;
	uint32_t  prevPage = currentPage;
	uint32_t lastMemIndex = startAddr;
	Serial.println("Checking Memory......");
	for (uint32_t index = startAddr; index <= endAddr; index++)
	{
		currentPage = index>>8;
		if(prevPage != currentPage)
		{
			memset(readPage,'\0',256);
			for(uint32_t pageMemIndex = lastMemIndex; pageMemIndex < index; pageMemIndex++)
			{
				readPage[pageMemIndex- lastMemIndex] = (uint8_t)pageMemIndex;
			}
			write(lastMemIndex, &writePage[0], (uint16_t)(index-lastMemIndex));
			delay(50);
			memset(writePage,'\0',256);
			read(lastMemIndex, &readPage[0], (uint16_t)(index-lastMemIndex));
			if(strncmp((const char*)writePage,(const char*)readPage,(size_t)(index-lastMemIndex)) == 0)
			{
				memoryOk &= true;
			}
			else
