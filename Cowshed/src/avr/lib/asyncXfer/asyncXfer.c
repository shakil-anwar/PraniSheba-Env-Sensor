    ...
    if (_isReady)
    {
        ...
        case READ_MEM:
            ...
            if (_ptr != NULL)
            {
                ...
                if(*_ptr == 0)
                {
                    // SerialPrintF(P("check mid ptr: "));
                    // SerialPrintlnU8(*_ptr);
                    sendState = READ_MEM;
                    break;
                }
#endif
...
#if defined(STAND_ALONE)
            else if(_ackCode == 404)
            {
               if(_debug) {SerialPrintlnF(P("Xfer :ACK_FAILED"));}
               sendState = FAILED; 
            }
            else
            {
        ...
        case SEND_SUCCESS:
            ...
            {
                #if defined(ESP32_V010)
                _totalTime = (uint32_t)millis() -  _startMillis;
