...
bool nrfQueryBeginClient(volatile struct qryObj_t *qryObj)
{
    ...
    SerialPrintF(P("Addr1 : ")); nrfPrintBuffer(qryObj -> activePingAddr, 5);
    SerialPrintF(P("Addr2 : ")); nrfPrintBuffer(qryObj -> pipe0AddrP, 5);
...
//   nrfNodeConfig_t addr;
  ...
  configPtr = nrfQuery(&query,(void *)configPtr,sizeof(nrfNodeConfig_t));
...
void nrfTxSetModeClient(enum addrMode_t addrMode,nrfNodeConfig_t *conPt)
{
    ...
    {
    case COMMON_PING:
        SerialPrintF(P("Addr common ping : ")); nrfPrintBuffer(nrfQryObj.activePingAddr, 5);
        // nrfQryObj.activePingAddr = pipe0AddrPtr;
        nrfQryObj.activePingAddr = nrfQryObj.activePingAddr;
        SerialPrintF(P("Addr common ping : ")); nrfPrintBuffer(nrfQryObj.activePingAddr, 5);
        SerialPrintF(P("Addr2 coom : ")); nrfPrintBuffer(nrfQryObj.pipe0AddrP, 5);
        break;
    case BS_PING:
        SerialPrintF(P("Addr BS ping : ")); nrfPrintBuffer(nrfQryObj.activePingAddr, 5);
...
uint8_t *nrfQuery(query_t *qry,void *bufPtr, uint8_t len)
{
    ...
    SerialPrintF(P("Sending : ")); nrfPrintBuffer(nrfQryObj.activePingAddr, 5);
    ...
    uint32_t prevMillis2 = currentMillis;
    ...
    {   
        ...
        if (queryPtr != NULL)
        {
            ...
            else
            {
                ...
                if(_nrfDebug)
                {
                    SerialPrintlnU32((uint32_t)queryPtr);
        ...
        #if defined(ESP32_V010)
        currentMillis = (uint32_t) millis();
        #else    
        currentMillis = _Millis();
        #endif
        
    } while ((currentMillis - prevMillis2) < QUERY_TIMEOUT_MS);
