...
#define NRF24_DRIVER_H_
    ...
    #include <Arduino.h>
        ...
        #include "../../../common_lib/arduinoCwrapper/spi_driver.h"
    #else
...
#if defined(ARDUINO_ARCH_AVR) || defined(__MSP430G2553__) || defined(ARDUINO_ARCH_ESP32)
...
typedef uint16_t gpio_pin;
    ...
    #define nrf_csn_low()     digitalWrite(_csnPin, LOW)
    #define nrf_csn_high()    digitalWrite(_csnPin,HIGH)

    #define nrf_ce_high()  digitalWrite( _cePin,HIGH)
    #define nrf_ce_low()   digitalWrite( _cePin,LOW)
#else
    ...
    #define nrf_ce_read() digitalRead(_cePin);
#else
