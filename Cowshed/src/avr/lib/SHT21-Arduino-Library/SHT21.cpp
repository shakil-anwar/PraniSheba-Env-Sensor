...
void SHT21::begin(uint8_t sda, uint8_t scl)
{
#if defined(SOFTWARE_I2C)
	softWire.begin(sda,scl);
	// Serial.println("[software i2c begin]");
#else
	Wire.begin();
	Serial.println("[hardware i2c begin]");
#endif
}
...
void SHT21::reset() {
#if defined(SOFTWARE_I2C)
	softWire.beginTransmission(I2C_ADD);
	softWire.write(SOFT_RESET);
...
uint8_t SHT21::getSerialNumber(uint8_t return_sn) {
	...
	softWire.requestFrom(I2C_ADD,8);
	while(softWire.available() < 8) {}

	serialNumber[5] = softWire.read();	// read SNB_3
	softWire.read();					// CRC SNB_3 not used
	serialNumber[4] = softWire.read();  // read SNB_2
	softWire.read();					// CRC SNB_2 not used
	serialNumber[3] = softWire.read();	// read SNB_1
	softWire.read();					// CRC SNB_1 not used
	serialNumber[2] = softWire.read();	// read SNB_0
	softWire.read();					// CRC SNB_0 not used
	...
	softWire.requestFrom(I2C_ADD,6);
	while(softWire.available() < 6) {}

	serialNumber[1] = softWire.read();	// read SNC_1
	serialNumber[0] = softWire.read();  // read SNC_0
	softWire.read();					// CRC SNC_1/SNC_0 not used
	serialNumber[7] = softWire.read();	// read SNA_1			
	serialNumber[6] = softWire.read();	// read SNA_0
	softWire.read();
#else
	Wire.beginTransmission(I2C_ADD);
	Wire.write(0xFA);
	...
	Wire.read();
#endif					// CRC SNA_1/SNA_0 not used
...
#if defined(SOFTWARE_I2C)	
	softWire.beginTransmission(I2C_ADD);
	softWire.write(command);
	softWire.endTransmission();
	delay(d);
	softWire.requestFrom(I2C_ADD,3);
	
	while(softWire.available() < 3) {
		delay(10);
		n++;
		if(n>100) return 0;
	}

	data[0] = softWire.read(); 	// read data (MSB)
	data[1] = softWire.read(); 	// read data (LSB)
	checksum = softWire.read();	// read checksum
#else 
	Wire.beginTransmission(I2C_ADD);
	Wire.write(command);
	Wire.endTransmission();
