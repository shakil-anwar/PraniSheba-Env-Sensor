...
const char ptext3[] PROGMEM = "3. URLTest";
...
void loadDummyConf(config_t *configPtr, int deviceID);
...
{
  read(&config);
  loadDummyConf(&config, SENSOR_ID); //(conf_ptr, device ID)
  configPrint(&config);
  /********************************************/
  // Serial.println("set config");
  // config.isSetupDone = SETUP_DONE_CONST;
  // config.isDebugOff = true;
  // config.deviceId = 3001;
  // config.sampInterval = 10;
  save(&config);
  /********************************************/
...
void printParamCmd(const char* const* text, uint8_t totalCmd) //const char** const
{
...
void loadDummyConf(config_t *configPtr, int deviceID)
{
  configPtr -> deviceId = deviceID;
  configPtr -> sampInterval = 30;
  configPtr -> isSetupDone = SETUP_DONE_CONST;


  bsParam.bsId = deviceID;
  bsParam.deviceType =  SENSOR_TYPE; 
  bsParam.bsStartingTime =  (uint32_t)millis();
  bsParam.bsServerRegTime = 0;
  bsParam.subscriptionStatus =  (uint8_t)false;
}
